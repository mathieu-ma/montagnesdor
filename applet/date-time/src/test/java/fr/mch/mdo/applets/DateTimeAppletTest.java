...
public class DateTimeAppletTest extends MdoTestCase {
	...
	applet.init();
	try {
	    // Key and DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY do not exist in the resource file
	    // Get the current properties file
	    properties = super.getProperties(clazz);
	    key = DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY;
	    ...
	    // Remove properties
	    // DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY does not exist anymore
	    updatePropertiesFile(clazz, properties, true);
	    key = "my.key";
	    // With default value
	    assertEquals("Method getResourceString must return a default message", String.format(DateTimeApplet.DEFAULT_RESOURCE_MESSAGE, key), invokeGetResourceString(applet, key, true));
	    // With No default value 
	    assertNull("Method getResourceString must return null", invokeGetResourceString(applet, key, false));
	    // Restore new properties
	    // DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY does exist again
	    updatePropertiesFile(clazz, properties, false);
	    // Key does not exist and DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY do exist in the resource file
	    key = DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY;
	    value = "My Value";
	    properties = new Properties();
	    properties.setProperty(key, value);
	    // Add key DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY to be sure that does exist
	    updatePropertiesFile(clazz, properties, false);
	    key = "my.key";
	    applet = new DateTimeApplet();
	    // Load resource properties if needed
	    ...
	    // DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY does exist again
	    properties = new Properties();
	    properties.setProperty(DateTimeApplet.DEFAULT_RESOURCE_MESSAGE_KEY, backupValue);
	    updatePropertiesFile(clazz, properties, false);
	} catch (InvocationTargetException e) {
	    fail("No Exception could be thrown");
	}
    ...
    private Object invokeGetParameter(DateTimeApplet applet, String key, String defaultValue)
    ...
    private Object invokeGetResourceString(DateTimeApplet applet, String key, boolean isDefaultValue)
    throws InvocationTargetException {
	// Purposely pass null values to the method, to make sure it throws
	// NullPointerException
	...
	//Test without DateTimeApplet.properties: remove all files DateTimeApplet.properties in the class path(2 files)
	DateTimeApplet applet = new DateTimeApplet();
	...
	applet.init();
	checkDefaultInitValue(applet);
	// Because there is no DateTimeApplet.APPLET_PARAMETER_LOCALE_LANGUAGE_KEY
	assertEquals("Checking the debug flag", true, applet.isDebug());
    }
	...
	try {
	    assertEquals("The entryDate must be equals to the current date without time", applet.getFormatApplicationDateShort().format(new Date()), applet.getFormatApplicationDateShort().format(applet.getFormatApplicationDateLong().parse(applet.getEntryDate())));
	...
	assertEquals("DateTimeApplet.formatUpperCase must return string with first letter in upper case", valueToBeFormatted.substring(0, 1).toUpperCase() + valueToBeFormatted.substring(1), DateTimeApplet.formatUpperCase(valueToBeFormatted));
    }
