 ...
 */
public class PrinterSimulator extends JFrame implements WindowListener,
    ...
    private static Logger logger = Logger.getLogger(PrinterSimulator.class
	    .getName());
    private static ResourceBundle resource;// = ResourceBundle.getBundle(PrinterSimulator.class.getName());
	...
	try {
	    ...
	    PatternLayout patternLayout1 = new PatternLayout(resource
		    .getString("log4j.appender.A1.layout.ConversionPattern"));
	    ConsoleAppender consoleAppender = new ConsoleAppender(
		    patternLayout1);
	    logger.addAppender(consoleAppender);

	    boolean flag = resource.getString("log4j.appender.A2.Append") == null ? false
		    : resource.getString("log4j.appender.A2.Append").equals(
			    "true");
	    String file = PrinterSimulator.class.getResource(
		    resource.getString("log4j.appender.A2.FileName")).getPath();
	    PatternLayout patternLayout2 = new PatternLayout(resource
		    .getString("log4j.appender.A2.layout.ConversionPattern"));
	    RollingFileAppender rollingFileAppender = new RollingFileAppender(
		    patternLayout2, file, flag);
	    rollingFileAppender.setName(resource
		    .getString("log4j.appender.A2.Name"));
	    rollingFileAppender.setMaxFileSize(resource
		    .getString("log4j.appender.A2.MaxFileSize"));
	    rollingFileAppender.setMaxBackupIndex(Integer.parseInt(resource
		    .getString("log4j.appender.A2.MaxBackupIndex")));
	    logger.addAppender(rollingFileAppender);

	    logger.setLevel(Level.toLevel((resource
		    .getString("log4j.rootLogger.level"))));
	} catch (Exception e) {
	    System.err
		    .println("Error loading Log4J properties : "
			    + e);
	}
	...
	try {
	    ...
	    logger
		    .fatal("Enable to load Serial Port Printer driver "
			    + e);
	}
    }
	...
	if (portId == null) {
	    ...
	    try {
			...
			logger.warn("Identifiant du port de communication trouve sur " + portCom);
		    }
		    catch (Exception e)
		    {
			...
			if (enumeration != null)
			{
				...
				if (searchedPortId.getPortType() == CommPortIdentifier.PORT_SERIAL)
				{
					...
					logger.info("Identifiant du port de communication trouve sur " + portId.getName());
					break;
				    }
				}
			    }
		    ...
		    if (flagNoPortComFound)
		    {
			logger.info("Impossible de recuperer de l'identifiant du port s�rie de communication apr�s " + attempt + " tentative(s)");
		    }
	...
	openSerialPort.addActionListener(this);
	...
	closeSerialPort.addActionListener(this);
	...
	if (portId != null) {
		...
		if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL) {
		    serialPort = (SerialPort) portId.open("montagnesdor",
			    Integer.parseInt(resource.getString("pause")) * 2);
		    serialPort.setSerialPortParams(Integer.parseInt(resource
			    .getString("serialportBauds")), Integer
			    .parseInt(resource.getString("serialportBits")),
			    Integer.parseInt(resource
				    .getString("serialportStopBits")), Integer
				    .parseInt(resource
					    .getString("serialportParity")));
		    ...
		    logger
			    .info("Time in milliseconds to block waiting for port open: "
				    + Integer.parseInt(resource
					    .getString("pause")) * 2);
		    logger.info("serialportBauds: "
			    + Integer.parseInt(resource
				    .getString("serialportBauds")));
		    logger.info("serialportBits: "
			    + Integer.parseInt(resource
				    .getString("serialportBits")));
		    logger.info("serialportStopBits: "
			    + Integer.parseInt(resource
				    .getString("serialportStopBits")));
		    logger.info("serialportParity: "
			    + Integer.parseInt(resource
				    .getString("serialportParity")));
		}
	    } catch (Exception e) {
	...
	// This is used to prevent null pointer exception for logging
	...
	case SerialPortEvent.OUTPUT_BUFFER_EMPTY:
	    logger
		    .info("SerialPortEvent.OUTPUT_BUFFER_EMPTY : "
			    + SerialPortEvent.OUTPUT_BUFFER_EMPTY
			    + " : taille du buffer "
			    + serialPort.getOutputBufferSize());
	...
	case SerialPortEvent.DATA_AVAILABLE:
	    ...
	    logger.info("SerialPortEvent.DATA_AVAILABLE : "
		    + SerialPortEvent.DATA_AVAILABLE);
	    try {
		BufferedInputStream bis = new BufferedInputStream(serialPort
			.getInputStream());
	...
	for (int i = 0; i < data.length; i++) {
		...
		text.append("<0x");
		text.append((data[i] < 16 ? "0" : "")
			+ Integer.toHexString(data[i]).toUpperCase());
		text.append(">");
	    } else {
