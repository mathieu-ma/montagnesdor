...
public class MdoSwitchSchemeInterceptor implements Interceptor 
{
	...
	public void init() {
		...
		if ((i = defaultEntryPatternWithMethodAction.lastIndexOf(".")) > 0) {
			defaultEntryPatternWithMethodAction = defaultEntryPatternWithMethodAction
					.substring(0, i)
					+ patternMethodAction
					+ defaultEntryPatternWithMethodAction.substring(i);
		}
	}
	...
	public String switchSchemeNone(ActionInvocation invocation)
			throws Exception {
		return null;
	}

	public String switchSchemeOnlyHttp(ActionInvocation invocation)
			throws Exception {
		String result = null;
		...
		String schemeHTTP = ServletActionContext.getServletContext()
				.getInitParameter(Constants.SCHEME_HTTP_KEY);
		String portHTTP = ServletActionContext.getServletContext().getInitParameter(Constants.PORT_HTTP_KEY);

		if (request.isSecure()) {
			String url = schemeHTTP + "://" + request.getServerName() + ":"
					+ portHTTP + request.getRequestURI();
	...
	public String switchSchemeOnlyHttps(ActionInvocation invocation)
			throws Exception {
		String result = null;
		...
		String schemeHTTPS = ServletActionContext.getServletContext()
				.getInitParameter(Constants.SCHEME_HTTPS_KEY);
		String portHTTPS = ServletActionContext.getServletContext()
				.getInitParameter(Constants.PORT_HTTPS_KEY);

		if (!request.isSecure()) {
			String url = schemeHTTPS + "://" + request.getServerName() + ":"
					+ portHTTPS + request.getRequestURI();
			if (request.getQueryString() != null) {
				url += "?" + request.getQueryString();
			}
			response.sendRedirect(url);
	...
	public String switchSchemeHttpsEntryPointOnly(ActionInvocation invocation)
			throws Exception {
		String result = null;
		...
		String schemeHTTP = ServletActionContext.getServletContext()
				.getInitParameter(Constants.SCHEME_HTTP_KEY);
		String portHTTP = ServletActionContext.getServletContext()
				.getInitParameter(Constants.PORT_HTTP_KEY);
		...
		if (request.isSecure()) {
			if (session.getAttribute(Constants.SCHEME_HTTP_KEY) == null) {
				...
				response.sendRedirect(schemeHTTP + "://"
						+ request.getServerName() + ":" + portHTTP
						+ defaultEntryURI);
				result = Action.NONE;
			}
			if (!request.getRequestURI().matches(
					defaultEntryPatternWithMethodAction)) {
			...
			if (request.getRequestURI().matches(
					defaultEntryPatternWithMethodAction)) {
				response.sendRedirect(schemeHTTPS + "://"
						+ request.getServerName() + ":" + portHTTPS
						+ defaultEntryURI);
				result = Action.NONE;
			}
		}
