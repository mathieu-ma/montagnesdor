...
<%@ taglib uri="/struts-tags" prefix="s" %>
	...
	<tiles:insertDefinition name="admin.manager">
		...
		<c:set var="bodyInclude">/jsp/references/MdoTableAsEnumsManager</c:set>
		<c:choose>
			<c:when test="${method=='form'}">
				<c:set var="bodyInclude"><c:out value="${bodyInclude}"/>/form-body.jsp</c:set>
			</c:when>
			<c:when test="${method=='listType'}">
				<c:set var="bodyInclude"><c:out value="${bodyInclude}"/>/list-type-body.jsp</c:set>
			</c:when>
