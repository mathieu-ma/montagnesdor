(function($){
	 ...
	 * var labelSubscriptionDuration = $.mdoI18n.prop("subscription.duration"); 
	 * 
	...
	jQuery.mdoI18n.properties = function(keys, completeCallback) {
		...
		if (keys && keys.keyArgsMap) {
			...
			$.each(keys.keyArgsMap, function(key, value) {
				...
				var cachedElement = $.mdoI18n.map[key];
	        	if (cachedElement && cachedElement.state == 1) {
	        		// Remove existed key
	        		delete keys.keyArgsMap[key];
	        		numberOfKeysToProcess--;
	        	} else {
	        		// processing
	        		cachedElement = {state: -1};
	        	}
	        });
