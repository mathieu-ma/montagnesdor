...
<s:form action="CategoriesManagerCUD" method="post" validate="false">
	...
	<div class="mdo-upsidedown">
		...
		<div class="hspacer-left-100p">
			<s:if test="%{form.dtoBean.id==null}">
				<s:select label="%{getText('categories.manager.code')}" name="form.dtoBean.code.id" list="form.viewBean.codes" listKey="id" listValue="name" value="form.dtoBean.code.id" required="true"/>
			</s:if>
			<s:else>
				<s:label label="%{getText('categories.manager.code')}" value="%{form.dtoBean.code.name}" required="true" />
