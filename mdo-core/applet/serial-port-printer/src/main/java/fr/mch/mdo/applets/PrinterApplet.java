...
public class PrinterApplet extends java.applet.Applet
{
    ...
    public static final byte[] PRINT_FEED_PAPER_CUT_SHEET =
    {
	    (byte) 0x1B, (byte) 0x4A, (byte) 0xF0, (byte) 0x1B, (byte) 0x69
    };
    // public static final char[] PRINT_FEED_PAPER_CUT_SHEET = {(char) 0x1B,
    // (char) 0x4A, (char) 0xF0, (char) 0x1B, (char) 0x69 };

    // Initialisation de l'imprimante, format de la police
    // Caracteres internationnaux : Français pour les accents
    public static final byte[] INITIALIZE_PRINTER_INTERNATIONAL_CHARACTER =
    {
	    (byte) 0x1B, (byte) 0x40, (byte) 0x1B, (byte) 0x52, (byte) 0x01
    };
     ...
     * the browser.
     * In the java.policy file we MUST be updated with the following lines:
	...
	if (driver == null)
	{
		...
		if (debug)
		{
		    System.out.println("Identifiant du port de communication trouv� sur " + portCom);
		}
	    }
	    catch (Exception e)
	    {
		...
		if (enumeration != null)
		{
			...
			if (searchedPortId.getPortType() == CommPortIdentifier.PORT_SERIAL)
			{
				...
				if (debug)
				{
				    System.out.println("Identifiant du port de communication trouv� sur " + portId.getName());
				}
				break;
			    }
			}
	    ...
	    if (flagNoPortComFound)
	    {
		System.out.println("Impossible de r�cup�rer de l'identifiant du port s�rie de communication apr�s " + attempt + " tentative(s)");
	    }
	    printer = new Printer();
	}
    }
	...
	if (size == 2)
	{
	    // Format de la police en gras et avec une taille double en largeur
	    // et en longueur
